...
async def read_csv(fname , sender : ServiceBusSender):
    ...
    with open(fname, newline='') as csvfile:
        ...
        for employee in employees:
            #print(employee)
            message1 = ServiceBusMessage(str(employee), content_type="application/json")
...
async def run():
    ...
    async with ServiceBusClient.from_connection_string(
        ...
        sender : ServiceBusSender = servicebus_client.get_queue_sender(queue_name=os.getenv('QUEUE_NAME'))
        async with sender:
            # Send one message            
